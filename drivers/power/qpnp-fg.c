...
static int get_sram_prop_now(struct fg_chip *chip, unsigned int type)
{
	...
	if (type == FG_DATA_BATT_TEMP) {
		static int i=0,count=0,temp = -100;
		if(count<1){
			count++;
			return temp;
		}else{
			...
			if(i<=670){
				i++;
				temp++;
				pr_info("BATT@ [force_get_tbat] temp=%d i=%d\n",temp,i);
			}else if(i<=1400){
				i++;
				temp--;
				pr_info("BATT@ [force_get_tbat] temp=%d i=%d\n",temp,i);
...
static void update_charge_current(struct work_struct *work)
{
	...
	if (is_usb_present(chip)) {
		if ((batt_temp <= 0) ||(batt_temp >= 550) || (stop_flag == 1)) {
		...
		if (err_bat_temp_state) {
			if (((batt_temp >= 50) && (batt_temp < 510) && (stop_flag == 0))||
				((stop_flag == 1) && ((batt_temp < 410) || hq_get_fg_data(chip, FG_DATA_VOLTAGE) <= 4000000))) {
				stop_flag = 0;
			...
			if ((batt_temp > 0) && (batt_temp <= 50))
				bat_temp_state = TEMP_POS_0_TO_POS_5;
			else if ((batt_temp > 50) && (batt_temp <= 100) )
				bat_temp_state = TEMP_POS_5_TO_POS_10;
			else if ((batt_temp > 100) && (batt_temp < 460) )
				bat_temp_state = TEMP_POS_10_TO_POS_45;
			else if ((batt_temp >= 460) && (batt_temp < 550) )
		...
		pr_info("err_bat_temp_state8=%d,bat_temp_state=%d,last_bat_temp_state=%d\n",err_bat_temp_state,bat_temp_state,last_bat_temp_state);
	}
#endif
