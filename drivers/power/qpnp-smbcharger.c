...
static struct device *condor_dev;
static struct class condor = {
	.name = "condor",
};
extern char *condor_flag;//Murphy add for Condor 1593
/* Mask/Bit helpers */
...
static ssize_t show_bootup_flag(struct device *dev,struct device_attribute *attr, char *buff)
{
	return sprintf(buff,"Condor_Flag\n");
}
static DEVICE_ATTR(bootup_flag,0444,show_bootup_flag,NULL);
static int proc_create_chg_enable(void)
{
	int rc = 0;
	rc = class_register(&condor);
	if(rc)
	{
		pr_warn("class register fail\n");
		return rc;
	}
	...
	condor_dev = kzalloc(sizeof(*condor_dev),GFP_KERNEL);
	if(IS_ERR(condor_dev)) {
		pr_warn("fail to create device\n");
		return -ENOMEM;
	}
	condor_dev->init_name = "bootup_flag";
	condor_dev->class = &condor;
	if(device_register(condor_dev)) {
		kfree(condor_dev);
		pr_warn("device register fail\n");
		return -1;
	}
	if(device_create_file(condor_dev,&dev_attr_bootup_flag)) {
		pr_warn("create device file fail\n");
		return -1;
	}
